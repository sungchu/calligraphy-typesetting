...
filter_calligrapher_input = st.text_input("指定特定書法家（若有多位，請用、分隔，留空則代表不指定）", "")
...
if st.button("開始搜尋"):
    ...
    for idx, word in enumerate(search_words):
        ...
        for j in j_elements[:-1]:
            ...
            if filter_calligrapher_list and (author_name not in filter_calligrapher_list):
                continue
...
if results:
    ...
    for w_idx, w in enumerate(search_words):
        ...
        selected_idx = image_select(
            ...
            use_container_width=False,  # 保持固定寬度
        )
